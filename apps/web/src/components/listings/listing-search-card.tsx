...
export function ListingSearchCard({ listing, compact = false }: ListingSearchCardProps) {
  ...
  const whatsappPhone = vendor.whatsappNumber?.replace('+', '');
  const whatsappText = whatsappPhone
    ? encodeURIComponent(
        `Hi, I found "${vendor.businessName}" on EventTrust Nigeria and I'm interested in "${listing.title}".`,
      )
    : null;
